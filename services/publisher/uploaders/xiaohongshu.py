"""Xiaohongshu (小红书) uploader — Playwright-based video upload automation."""
import asyncio
...
class XiaohongshuUploader(BaseUploader):
    ...
    async def upload(
        ...
        try:
            ...
            title_input = page.locator('[class*="title"] input, #title').first
            if await title_input.count() > 0:
                await title_input.fill(title)

            desc_area = page.locator('[class*="desc"] textarea, #desc').first
